...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        ItemTouchHelper(object : ItemTouchHelper.SimpleCallback(0, ItemTouchHelper.LEFT or ItemTouchHelper.RIGHT) {
            ...
            override fun onSwiped(viewHolder: RecyclerView.ViewHolder, swipeDir: Int) {
                ...
                doAsync {
                    val position = viewHolder.adapterPosition   // PRIMERO RECUPERAMOS LA POSICION DEL ELEMENTO QUE SE ELIMINÓ DE LA LISTA

                    // SE RECUPERAN TODAS LAS TAREAS
                    val tasks = viewAdapter.getTasks()

                    // SE HACE LA AFECTACION DE ELIMINACION DE LA BASE DE DATOS
                    AppDatabase.getInstance(this@MainActivity)?.taskDao()?.deleteTask(tasks[position])

                    // SE ACTUALIZA EL RECYCLERVIEW PARA CONTRAER LA LISTA
                    retrieveTasks()
                }.execute()
    ...
    private fun onItemClickListener(task: TaskEntry) {
        ...
        // EL ELEMENTO QUE SELECCIONÓ EL USUARIO NOS PASARÁ TODA LA INFORMACIÓN PERO SOLO PASAREMOS EL ID
    ...
    private fun retrieveTasks() {
        doAsync {
            ...
            // SE EJECUTA EN EL MISMO HILO DE LA INTERFACE GRAFICA
            runOnUiThread {
